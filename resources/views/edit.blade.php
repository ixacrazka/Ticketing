...
<div class="flex items-center justify-center mt-20 bg-gray-100">
    <div class="w-full max-w-2xl p-10 px-10 py-16 bg-indigo-950 rounded-3xl shadow-lg mx-auto">
        <h1 class="mb-8 text-3xl font-bold text-center text-white">Edit Jenis Pengaduan</h1>
        ...
        <form action="{{ route('jenis.update', $jenis->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="jenis_pengaduan" id="jenis_pengaduan" value="{{ old('jenis_pengaduan', $jenis->jenis_pengaduan) }}" class="w-full p-4 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-neon focus:border-neon" placeholder="Masukan Jenis" required>
            </div>
